...
class _NoSizeChunkReader:
    ...
    def read(self) -> str:
        if self.pos >= len(self.text):
            return ""
        end = min(len(self.text), self.pos + self.chunk_size)
        chunk = self.text[self.pos : end]
        self.pos = end
        return chunk


class _NoSizeErrorReader:
    def read(self) -> str:  # noqa: ARG002
        raise RuntimeError("boom no-arg")


def test_read_yaml_streaming_no_size_read_uses_fallback():
    text = "foo: 1\nbar: true\n"
    reader = _NoSizeChunkReader(text, chunk_size=3)
    assert yaml12.read_yaml(reader) == {"foo": 1, "bar": True}


def test_read_yaml_streaming_no_size_read_error_propagates():
    with pytest.raises(RuntimeError, match="boom no-arg"):
        yaml12.read_yaml(_NoSizeErrorReader())
